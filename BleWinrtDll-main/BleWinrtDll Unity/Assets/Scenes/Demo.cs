            ...
            do
            {
                ...
                if (status == BleApi.ScanStatus.AVAILABLE)
                {
                    if (!devices.ContainsKey(res.id)) { 
                        ...
                        if (res.name != "" ) { //&& res.isConnectable.ToString() == "True"
                            GameObject g = Instantiate(deviceScanResultProto, scanResultRoot);
                            g.name = res.id.ToString();
                            g.transform.GetChild(0).GetComponent<Text>().text = res.name;
                            g.transform.GetChild(1).GetComponent<Text>().text = res.id;
                            g.transform.GetChild(2).GetComponent<Text>().text = res.isConnectable.ToString();
                        }
                    }
                    if (res.nameUpdated)
                    ...
                    //Debug.Log(res.name + " " + res.isConnectable.ToString());
                    // consider only devices which have a name and which are connectable
                    if (res.isConnectableUpdated) { //res.nameUpdated ||
                        for (int i = 0; i <= scanResultRoot.childCount -1; i++) {
                            if (scanResultRoot.GetChild(i).name == res.id.ToString()){
                                //scanResultRoot.GetChild(i).GetChild(0).GetComponent<Text>().text = res.name;
                                scanResultRoot.GetChild(i).GetChild(2).GetComponent<Text>().text = res.isConnectable.ToString();
                            }
                        }
                    }
                }
